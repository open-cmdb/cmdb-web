<template>
  <el-dialog width="440px" title="重置密码" :visible.sync="is_visible" @close="on_close">
    <el-form label-width="100px" style="margin-right: 50px">
      ...
      <el-form-item label="验证码" :required="true" :error="String(errors.verify_code?errors.verify_code:'')">
        <el-input v-model="form_data.verify_code" style="width: 125px"></el-input>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      send_verify_code(){
        master.post("mgmt/user/send-verify-code", {username: this.form_data.username}).then((response)=>{
      ...
      submit(){
        master.post("mgmt/user/reset-password-email", this.form_data).then((response)=>{
