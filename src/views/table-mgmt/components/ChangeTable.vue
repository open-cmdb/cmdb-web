<template>
  <el-dialog width="1020px" :title="data?'修改表 '+data.name:'创建表'" :visible.sync="is_visible" @close="on_close(true)">
    <el-form size="small" label-width="60px" style="margin-right: 30px">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="图标:" :error="String(errors.icon?errors.icon:'')">
    ...
    <el-form v-for="(item, index) in form_data.fields" :key="index" size="small" :inline="true" label-width="60px">
      ...
      <el-form-item style="width: 140px"
                    :error="String(errors.fields&&errors.fields[index].name?errors.fields[index].name:'')">
        <el-input v-model="item.name" placeholder="字段名"></el-input>
      </el-form-item>
      <el-form-item style="width: 100px"
                    :error="String(errors.fields&&errors.fields[index]['alias']?errors.fields[index]['alias']:'')">
        <el-input v-model="item.alias" placeholder="别名"></el-input>
      </el-form-item>
      <el-form-item style="width: 100px"
                    :error="String(errors.fields&&errors.fields[index].type?errors.fields[index]['type']:'')">
      ...
      <el-form-item style="width: 80px"
                    :error="String(errors.fields&&errors.fields[index].width?errors.fields[index]['width']:'')">
      ...
      <el-form-item>
        <el-button size="mini" type="primary" style="margin: 0px 1px" icon="el-icon-plus"
                   @click="on_add_field(item, index)"></el-button>
        <el-button size="mini" type="danger" style="margin: 0px 1px" icon="el-icon-delete"
                   @click="on_delete_filed(item, index)"></el-button>
      </el-form-item>
    </el-form>
    <div v-show="form_data.fields.length==0">
      <el-button size="mini" type="primary" style="margin: 10px 60px" icon="el-icon-plus" @click="on_add_field({}, 0)">
        添加字段
      </el-button>
    </div>
