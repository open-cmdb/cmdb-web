...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      get_sim_data(){
        if(this.iccid){
          master.get(`sim/${this.iccid}`).then((response)=>{
            this.data.sim = response.data
          }).catch((error)=>{
          })
          return
        }
        this.$c_master.get(`sim?id=${this.sim_id}`).then((response)=>{
          var results = response.data.results
          if(results.length<1){
            this.$message.warning("抱歉，没有这个卡的相信信息")
            return
          }
          this.data.sim = response.data.results[0]
        }).catch((error)=>{
        })
      },
      get_flow_data(){
